...
const formAuth = new Auth(myErrors);

document.getElementById('login').addEventListener('click', function(event) {
  event.preventDefault();
  formAuth.login(getPosts);
});

async function getPosts() {
  try {
    const data = await makeRequest('posts', 'GET', null, formAuth.token);
...
async function createPost() {
  ...
  if (form.title.validity.valid && form.content.validity.valid) {
    ...
    try {
      const res = await makeRequest('posts', 'POST', data, formAuth.token);
